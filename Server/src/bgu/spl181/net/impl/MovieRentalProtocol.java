...
public class MovieRentalProtocol extends UserServiceTextBaseProtocol {
    ...
    public void process(String message) {
       super.process(message);
        if(msg.get(0)!=null&&msg.remove(0).equals("REQUEST")){
            switch ((!msg.isEmpty()? msg.get(0):"" )){
                case "balance":{
                    switch ((!msg.isEmpty()? msg.remove(0):"" )){
                        case "info":{
                            msg.remove(0);
                            response= balanceInfoProcess();
                        }
                        case "add":{
                            msg.remove(0);
                            response= balanceAddProcess();
                        }
                    }
                }
                case "info": {
                    response= infoProcess();
                }
                case  "rent" :{
                    response= rentProcess();
                }
                case "return":{
                    response= returnProcess();
                }
                case "addmovie" :{
                    response= addMovieProcess();
                }
                case "remmovie":{
                    response= remMovieProcess();
                }
                case "changeprice":{
                    response= changePriceProcess();
                }
                case "REGISTER":{
    ...
    private String balanceAddProcess(){
        ...
        int amount= Integer.getInteger(msg.remove(0));
        String [] userDetails= new String[2];
        userDetails[0]=sharedProtocolData.returnName(connectionId);
        usersLock.writeLock().unlock();
        ArrayList<User> users = sharedProtocolData.getUsers();
        users.forEach(user -> {
